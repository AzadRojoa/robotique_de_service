...
class ArucoDetectorNode(Node):
    ...
    def listener_callback(self, msg):
        ...
        # Appliquer un filtre morphologique pour améliorer la détection
        kernel = np.ones((5, 5), np.uint8)
        mask_cleaned = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel, iterations=2)
    
        # Trouver les contours des cartons
        contours, _ = cv2.findContours(mask_cleaned, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        for contour in contours:
            if 1000 < cv2.contourArea(contour) < 50000:  # Filtrer petits et très grands objets
                x, y, w, h = cv2.boundingRect(contour)
            
                # Vérifier la forme rectangulaire
                aspect_ratio = w / float(h)
                if 0.7 < aspect_ratio < 1.5:
                    cv2.rectangle(cv_image, (x, y), (x + w, y + h), (0, 255, 0), 2)
                    cv2.putText(cv_image, "Carton", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
